...
1. find out the device, for:
   ...
   1. [`pvc.test.file.yaml`](pvc.test.file.yaml) its: `/data`

   ```bash
   KE_DEVICE=/data/mounted-file-test
   ```

   1. local file at pod/vm [vm.test.attached-disk.ubuntu.yaml](vm.test.attached-disk.ubuntu.yaml):

   ```bash
   KE_DEVICE=./local-file-test
   ```

   1. mounted block VM [vm.test.attached-disk.ubuntu.yaml](vm.test.attached-disk.ubuntu.yaml):
...
| gcp   | kubev | Windows Server VM no guest utils, bus `sata`          | ceph replica 3, kubev-vms       | 5.9MiB/s  | 1475   | 5.9MiB/s  | 1475   |      |
